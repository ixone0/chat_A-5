          ...
          <div className="ac-video-box">
            <div className="ac-header">
              ...
              <div className="ac-type-badge">
                ...
                Video{activeCall.is_group ? ` · ${callParticipants.length}` : ''}
              </div>
            </div>
            {activeCall.is_group ? (
              <div className={`video-grid ${Object.keys(remoteStreams).length === 0 ? 'one-video' : Object.keys(remoteStreams).length === 1 ? 'two-videos' : 'multi-video'}`}>
                ...
                {Object.entries(remoteStreams).map(([uid, stream]) => {
                  ...
                  return (<div key={uid} className="video-container"><video autoPlay playsInline ref={el => { if (el && el.srcObject !== stream) el.srcObject = stream; }} /><div className="video-label">{p?.display_name || p?.username || 'User'}</div></div>);
                })}
              </div>
            ) : (
              <div className="ac-direct-video"><div className="ac-remote-video" ref={remoteVideoRef} /><div className="ac-local-video" ref={localVideoRef} /></div>
            )}
            <div className="ac-controls">
              ...
              <button onClick={toggleCamera} className={`ac-ctrl-btn ${isCameraOff ? 'active-danger' : ''}`} title={isCameraOff ? 'Camera on' : 'Camera off'}>
                {isCameraOff ? (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="1" y1="1" x2="23" y2="23"/><path d="M21 7l-5 3.5"/><path d="M16 16V5a2 2 0 0 0-2-2H3a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h11"/><path d="M23 7v10"/></svg>
              ...
              <button onClick={() => activeCall.is_group ? leaveCall(activeCall.call_id) : endCall(activeCall.call_id)} className="ac-ctrl-btn end-call" title={activeCall.is_group ? 'Leave' : 'End'}>
